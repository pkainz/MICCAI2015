...
TClassificationForest* loadForest(AppContextCellDetClass* apphp){
    ...
    // fix the number of classes to 2
    apphp->num_classes = 2;

    // Loading the forest
    if (!apphp->quiet)
        std::cout << "Loading the forest ... ";
    TClassificationForest* rf;
    switch (apphp->method)
    {
    case RF_METHOD::STDRF:
        rf = new TClassificationForest(rfparams, apphp);
        break;
    default:
...
void train(AppContextCellDetClass* apphp, std::vector<boost::filesystem::path>& trainImgFilenames)
{
    ...
    DataLoaderCellDetClass mydataloader(apphp);
    DataSet<SampleImgPatch, LabelMLClass> dataset_train = mydataloader.LoadTrainData(trainImgFilenames);

    int num_train_samples, num_classes, num_feature_channels, num_fg, num_bg;
    mydataloader.GetTrainDataProperties(num_train_samples, num_classes, num_feature_channels, num_fg, num_bg);
    ...
    if (!apphp->quiet)
    {
        ...
        std::cout << "(FG: " << num_fg << "/BG: " << num_bg << "), ";
        std::cout << num_classes << "-class classification" << std::endl;
    }
    ...
    {
        cout << "Clearing image_feature_data vector" << endl;
        DataLoaderCellDetClass::image_feature_data.clear();
        std::vector<std::vector<cv::Mat> > tmp;
        tmp.swap(DataLoaderCellDetClass::image_feature_data);
    }
}

// load a trained forest to predict the test images
void predictTestImages(AppContextCellDetClass* apphp){
    ...
    // cell centers
    CellDetectorClass detector(rf,apphp);
...
void analyze_leaves(AppContextCellDetClass* apphp)
{
    TClassificationForest* rf;
    rf = loadForest(apphp);
...
void analyze_splitnodes(AppContextCellDetClass* apphp)
{
    TClassificationForest* rf;
    rf = loadForest(apphp);
 ...
 * @param apphp
 */
void loocv(AppContextCellDetClass* apphp){
...
int main(int argc, char* argv[])
{
    ...
    AppContextCellDetClass apphp; // classification detection
