#ifndef APPCONTEXTCELLDETCLASS_H
#define APPCONTEXTCELLDETCLASS_H
...
class AppContextCellDetClass : public AppContext
{
public:
    AppContextCellDetClass() {}
    virtual ~AppContextCellDetClass() {}

    // Method for printing the Hyperparameters
    friend std::ostream& operator<< (std::ostream &o, const AppContextCellDetClass& apphp)
    {
        o << "==============================================================" << std::endl;
        o << "Application Context Parameters (CELL DETECTION CLASSIFICATION)" << std::endl;
        o << "==============================================================" << std::endl;
        ...
        o << "split_eval_type_class    = ";
        switch(apphp.splitevaluation_type_classification){
        ...
        o << "patch_size               = " << apphp.patch_size[0] << "x" << apphp.patch_size[1] << std::endl << std::endl;
...
protected:
    ...
    inline void ValidateHyperparameters()
    {
        ...
        if (this->splitevaluation_type_classification == SPLITEVALUATION_TYPE_CLASSIFICATION::NOTSET){
            cout << "split_evaluation_type_classification is missing!" << endl;
            exit(-1);
        }
        ...
        if (!this->ValidateCellDetectionClassification()){
            cout << "Some cell detection CLASSIFICATION parameters are missing!" << endl;
            exit(-1);
        }
    ...
    inline bool ValidateCellDetectionClassification()
    {
...
#endif // APPCONTEXTCELLDETCLASS_H
